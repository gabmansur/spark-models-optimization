...
def main():
    ...
    customers = (
        spark.range(0, N_CUSTOMERS)
             ...
             .withColumn("segment", F.when((F.col("customer_id") % 10) < 2, F.lit("VIP")).otherwise("STD"))
             .withColumn("signup_date", F.expr("date_sub(current_date(), cast(rand()*1800 as int))"))
    )
    ...
    for d in range(14):
        ...
        # Split daily rows into skewed vs normal
        ...
        skew = (
            spark.range(0, skew_count)
                 ...
                 .withColumn("event_ts", F.expr(f"to_timestamp('{day} 12:00:00')") +
                                       F.expr("INTERVAL cast(rand()*43200 as INT) SECONDS"))
        )
        ...
        norm = (
            spark.range(0, norm_count)
                 ...
                 .withColumn("amount", (F.rand() * 200).cast("double"))
                 .withColumn("event_ts", F.expr(f"to_timestamp('{day} 00:00:00')") +
                                       F.expr("INTERVAL cast(rand()*86400 as INT) SECONDS"))
        )
